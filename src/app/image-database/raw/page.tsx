...
export default async function RawImagesPage() {
  const imageDir = path.join(process.cwd(), "../../api/image_database/raw");
  ...
  try {
    ...
    imageUrls = files
      ...
      .map((file) => `/image-database/raw/${file}`);
  } catch (e) {
    console.error(e);
  ...
  return (
    <>
      <h1 className="text-2xl font-bold mb-4">Raw Images</h1>
      {error ? (
